...
import json
import asyncio
from pymodbus.server import StartAsyncTcpServer  # Use async version of the server
...
async def updating_writer(context, sock):
    ...
    sock.sendall(('{"request":"write","data":{"inputs":{"f2_valve_sp":' + repr(current_command) + '}}}\n').encode())
...
async def run_update_server():
    ...
    # run the server
    # ----------------------------------------------------------------------- #
    await StartAsyncTcpServer(context, identity=identity, address=("192.168.95.11", 502))

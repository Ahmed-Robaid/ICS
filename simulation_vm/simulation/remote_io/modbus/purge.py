...
async def updating_writer(context, sock):
    ...
    while True:
        ...
        sock.sendall(('{"request":"write","data":{"inputs":{"purge_valve_sp":'+repr(current_command)+'}}}\n').encode())
        try:
...
async def run_update_server():
    ...
    # ----------------------------------------------------------------------- #
    await StartAsyncTcpServer(context=context, identity=identity, address=("192.168.168.12", 502))
